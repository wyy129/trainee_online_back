package com.example.trainee_online_back.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.example.trainee_online_back.entity.Major;
import com.example.trainee_online_back.service.MajorService;
import com.example.trainee_online_back.mapper.MajorMapper;
import org.springframework.stereotype.Service;

/**
* @author wyy
* @description 针对表【major(专业表)】的数据库操作Service实现
* @createDate 2022-05-11 21:06:56
*/
@Service
public class MajorServiceImpl extends ServiceImpl<MajorMapper, Major>
    implements MajorService{

}




